...
const SidebarContainer = tw.div`absolute top-0 left-0 h-screen w-[300px] z-10 border-r `
        ...
        <div className="w-full">
          ...
          <div className="flex flex-row flex-nowrap max-w-full w-full overflow-scroll p-5 " css={[tw`border-2`]}>
            {numbers.map(number => (
              <div
                key={number}
                className={`flex min-w-[40px] min-h-[40px] items-center justify-center border border-gray-200 ${selectedNumber === number ? 'bg-gray-300' : ''}`}
